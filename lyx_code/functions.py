...
def train(model, train_data_loader, valid_data_loader, epoch_num, lr, layers):
    ...
    torch.save(model, '../trained_models/resnet'+str(layers)+'_'+str(epoch_num)+'.pth')


def auto_train(model, train_data_loader, valid_data_loader, epoch_num, lr, layers):
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=lr)
    ...
    torch.save(model, '../trained_models/resnet'+str(layers)+'_auto.pth')
